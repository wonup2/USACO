import java.io.*;
import java.util.*;
public class FEB_SIL_countcross {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(Arrays.deepToString(badFields));
	...
	public static void dfs(boolean[][] reachable, int x, int y) {
		...
		for(int k = 0; k < dx.length; k++) {
			int nx = x + dx[k];
			int ny = y + dy[k];
			System.out.println("before: "+badFields[x][y] + " " + new State(nx, ny));

			if(!badFields[x][y].contains(new State(nx, ny))) {
				System.out.println("after: "+ badFields[x][y] + " " + new State(nx, ny));
				dfs(reachable, nx, ny);
			}
		}
	}
